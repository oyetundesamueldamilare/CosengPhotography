...
using Resend; // Core SDK Namespace

namespace CosengPhotography.Services
{
    public class EmailService : IEmailService
    {
        // FIX 1: Use the standard SDK interface instead of the concrete class
        private readonly IResend _resendClient;
        private readonly EmailSettings _emailSettings;
        private readonly ILogger<EmailService> _logger;

        // FIX 2: Inject IResend into the constructor matrix
        public EmailService(IResend resendClient, IOptions<EmailSettings> emailSettings, ILogger<EmailService> logger)
        {
        ...
        public async Task SendEmailAsync(string toEmail, string subject, string body)
        {
            try
            {
                // FIX 3: Map to EmailMessageCommand as required by the Resend SDK
                var message = new EmailMessage
                {
                    From = $"{_emailSettings.SenderName} <{_emailSettings.SenderEmail}>",
                    To = { toEmail }, // Collection initializer syntax for the destination list
                    Subject = subject,
                    HtmlBody = body
                };

                // FIX 4: Invoke EmailSendAsync directly from the interface instance
                var response = await _resendClient.EmailSendAsync(message);

                // The response yields a Guid value representing the unique Message Identifier
                _logger.LogInformation("Email successfully sent to {Email}. Resend ID: {Id}", toEmail, response);
            }
            catch (Exception ex)
            {
        ...
        {
            _logger.LogInformation("Preparing gallery access email for client: {Email}", notification.CustomerEmail);
